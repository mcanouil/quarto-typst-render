...
Values can be Typst colour literals, CSS hex strings (converted automatically), or `auto` (reads from `_brand.yml`).
...
Set `output-directory` to also save copies to a predictable location.
Use `output-filename` to override the auto-generated filename.
...
Inline with explicit alt text: `{typst} $pi r^2$`{alt="pi r squared"} for the area of a circle.
