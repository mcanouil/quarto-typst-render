...
| `background`, `foreground` | `none` | A Typst colour, `auto` to read `_brand.yml`, or a map with `light` and `dark` keys. |
...
| Option | Default | Description |
| --- | --- | --- |
| `typst` | | The output format for this block alone. |
...
The global options above also apply per block, apart from `math`, `root`, `font-path`, and `package-path`, which are global only.
...
The global options apply; the block-only ones do not, which rules out `echo`, `eval`, `label`, `cap`, `alt` as an option, `file`, `output-filename`, `pages`, `layout-ncol`, and `output-location`.
