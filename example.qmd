...
Here is a red word `#text(red)[hello]`{typst} in the middle of a sentence.

Inline maths: `$ x^2 + y^2 = z^2 $`{typst} renders as a formula image.
...
For inline expressions (`` `...`{typst} ``), only `output`, `format`, `dpi`, `preamble`, `background`, `classes`, and `input` are supported as per-element attributes.
Block-only options (`width`, `height`, `margin`, `echo`, `eval`, `label`, `cap`, `alt`, `file`, `pages`, `layout-ncol`, `output-location`) are not applicable to inline expressions.
