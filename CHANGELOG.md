...
- fix: Warn when knitr takes the options of a `{typst}` chunk, such as `#|` lines or chunk header options, which the filter then never sees. The warning names each lost option and points to the `//|` syntax.
...
- docs: Split the examples of the documentation site into one page for each topic, and add pages for maths, cross-references, data from R and Python, and presentations. (#105)
...
- feat: Check the document configuration against the extension schema and report what it does not accept. (#93)
- feat: Bind the document brand as `_typst_render_brand` in every block and inline expression, in the `_brand.yml` shape. Every cached image compiles again on the first render after this change. (#91)
